  ...
  (blocks||[]).forEach(b => {
    switch(b.type){
      ...
      case 'item': {
        ...
        el.innerHTML = `<div class="n">${esc(b.fr)}${b.en?'<span class="en">'+esc(b.en)+'</span>':''}</div><div class="dots"></div>${b.price?'<div class="p">'+esc(b.price)+'</div>':''}`;
        section.appendChild(el); break;
      }
      case 'formule': {
        const el = document.createElement('div'); el.className='menu-formule'+(b.heading?' heading':''); el.textContent=b.text; section.appendChild(el); break;
      }
      case 'note': {
